...
import { ensureConfigDir, CONFIG_PATH, describeConfigLocation, loadConfig } from "./config.js";
...
function parseArgs(argv: string[]): CliArgs {
  const args: CliArgs = { json: false, suite: false, initConfig: false, help: false };
...
async function main(): Promise<number> {
  ...
  if (args.initConfig) {
    ...
    process.stdout.write(`当前生效配置：${JSON.stringify(loadConfig(), null, 2)}\n`);
    return 0;
  }
  if (!args.input) {
    process.stdout.write(HELP + "\n");
