...
命令：`/status`（查看运行队列）、`/cancel <n>`（取消一个尚未开始的待办）、`/model`（选择本次会话使用的模型）、`/login`（登录一个 provider）、`/logout`（移除某 provider 的本地凭据）、`/toggle-language`（切换界面语种 zh/en，并把 `locale` 写入 `~/.pageqa/config.json`，下次启动生效）、`/help`、`/exit`。在输入行首键入 `/` 会弹出命令联想列表（支持模糊过滤；`/cancel` 还会补全待办编号，Tab 可补全）。
